...
Display( )
{
	...
	if (IsSpotLight == true) {
		...
		// glRotatef(deltaAngle, 0.0f, 1.0f, 0.0f); // Revolve in a circle => 360.f*Time

		SetSpotLight( GL_LIGHT0, 0., 2., 0., deltaY, -5., 0., h_r, h_g, h_b );	
	}
	else {
		SetPointLight( GL_LIGHT0, deltaY, 2., 0., h_r, h_g, h_b );	
	}

	glPushMatrix( );
		glTranslatef(deltaY, 1.5, 0.);
		glScalef(.05, .05, .05);
		glColor3f( h_r, h_g, h_b );
		glCallList( H_Sphere );
	glPopMatrix( );
